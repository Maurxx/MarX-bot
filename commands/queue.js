...
module.exports = {
  ...
  run: async (client, message, args, { GuildDB }) => {
    ...
    if (!player.queue || !player.queue.length || player.queue === 0) {
      let QueueEmbed = new MessageEmbed()
        ...
        .addField("Propuesta por", `${player.queue.current.requester}`, true)
        .addField(
          "Duración",
          `${
            client.ProgressBar(
  ...
  SlashCommand: {
    ...
    run: async (client, interaction, args, { GuildDB }) => {
      ...
      if (!player)
        return interaction.send("❌ | **Nada se está reproduciendo ahora mismo...**");

      if (!player.queue || !player.queue.length || player.queue === 0) {
        let QueueEmbed = new MessageEmbed()
          .setAuthor("Ahora reproduciendo", client.config.IconURL)
          .setColor("RANDOM")
          .setDescription(
            `[${player.queue.current.title}](${player.queue.current.uri})`
          )
